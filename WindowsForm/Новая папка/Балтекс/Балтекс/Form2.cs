    ...
    public partial class Form2 : Form
    {
        ...
        private void LoadProductsData()
        {
            using (SqlConnection conn = new SqlConnection(connection))
            {
                ...
                {
                    conn.Open();
                    string query = "SELECT * FROM Продукция ";
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //Запрос
                string query = "INSERT INTO Продукция (Артикул, Наименование, Тип_ткани, Ширина, Плотность, Состав, Цена, Количество_на_складе, Минимальный_запас) VALUES (@Value1,@Value2,@Value3,@Value4,@Value5,@Value6,@Value7,@Value8,@Value9)";
                using (SqlConnection conn = new SqlConnection(connection))
                {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            {
                //Запрос
                string query = "delete from Продукция where Код_Продукта = @Value1";
                using (SqlConnection conn = new SqlConnection(connection))
                {
                    conn.Open();
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(lastName))
            {
                using (SqlConnection conn = new SqlConnection(connection))
                {
                    ...
                    {
                        conn.Open();
                        string query = "SELECT * FROM Продукция WHERE Наименование LIKE @LastName + '%'";
                        SqlDataAdapter adapter = new SqlDataAdapter(query, connection);
        ...
        private string GetSortField()
        {
            if (comboBoxSortField.SelectedItem != null)
            {
                ...
                if (selectedField == "Артикул")
                    return "Артикул";
                else if (selectedField == "Наименование")
                    return "Наименование";
                else if (selectedField == "Тип ткани")
                    return "Тип_ткани";
                else if (selectedField == "Цена")
                    return "Цена";
                else if (selectedField == "Остаток на складе")
                    return "Количество_на_складе";
            }
            return "Артикул"; // Значение по умолчанию
        }
