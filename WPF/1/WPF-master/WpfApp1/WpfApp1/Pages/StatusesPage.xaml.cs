...
namespace WpfApp1.Pages
{
    public partial class StatusesPage : Page
    {
        ...
        private void BtnDelete_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                var result = MessageBox.Show($"Вы уверены, что хотите удалить статус '{selectedStatus.Name}'?", "Подтверждение", MessageBoxButton.YesNo);
                if (result == MessageBoxResult.Yes)
                {
                    ...
                    {
                        MessageBox.Show($"Ошибка удаления (возможно статус используется): {ex.Message}");
                    }
                }
            }
            else
            {
                MessageBox.Show("Сначала выберите статус для удаления.");
            }
        }
