...
namespace WpfApp1.Pages
{
    public partial class EmployeesPage : Page
    {
        ...
        private bool FilterEmployees(object item)
        {
            ...
            {
                return e.FullName.ToLower().Contains(txtSearch.Text.ToLower()) || 
                       (e.Position != null && e.Position.ToLower().Contains(txtSearch.Text.ToLower())) ||
                       (e.Department != null && e.Department.ToLower().Contains(txtSearch.Text.ToLower()));
            }
            return false;
        }
        ...
        private void BtnDelete_Click(object sender, RoutedEventArgs e)
        {
            if (EmployeesGrid.SelectedItem is Employee selectedEmployee)
            {
                ...
                if (MessageBox.Show($"Удалить '{selectedEmployee.FullName}'?", "Вопрос", MessageBoxButton.YesNo) == MessageBoxResult.Yes)
                {
