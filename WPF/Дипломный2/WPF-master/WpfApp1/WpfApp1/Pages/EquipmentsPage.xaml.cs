     ...
     .Load();
        ...
        private bool FilterEquipments(object item)
        {
            if (item is Equipment eq)
            {
                if (!string.IsNullOrWhiteSpace(txtSearch.Text))
                {
                    ...
                    bool matchLocation = eq.Location != null && eq.Location.ToLower().Contains(search);

                    bool matchEmployee = eq.Employee != null && eq.Employee.FullName != null && eq.Employee.FullName.ToLower().Contains(search);

                    bool matchDepartment = eq.Employee != null && eq.Employee.Department != null && eq.Employee.Department.ToLower().Contains(search);

                    if (!matchName && !matchInv && !matchSerial && !matchLocation && !matchEmployee && !matchDepartment)
                        return false;
                }
        ...
        private void BtnAdd_Click(object sender, RoutedEventArgs e)
        {
            ...
            var newEquipment = new Equipment
            {
                ...
                ServiceLifeYears = int.TryParse(txtServiceLife.Text, out int sl) ? sl : (int?)null
            };
            ...
            {
                if (ex.InnerException?.Message.Contains("IX_Equipments_InventoryNumber") == true ||
                    ex.InnerException?.Message.Contains("UNIQUE constraint") == true)
                {
                    MessageBox.Show("Ошибка: такой инвентарный номер уже занят.", "Ошибка", MessageBoxButton.OK, MessageBoxImage.Error);
                }
                else
                {
        ...
        private void BtnDelete_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                var result = MessageBox.Show($"Вы уверены, что хотите СПИСАТЬ '{selectedEquipment.Name}'?\nЗапись останется в архиве, но статус сменится на '{SystemStatuses.Scrapped}', а ответственный будет снят.",
                ...
                if (result == MessageBoxResult.Yes)
                {
                    try
                    {
                        ...
                        if (scrappedStatus != null)
                        {
                            ...
                            selectedEquipment.Location = "Склад списания";
        ...
        private void BtnSave_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                foreach (var eq in localData)
                {
                    ...
                    if (statusName != null)
                    {
                        if (statusName == SystemStatuses.InUse && eq.EmployeeId == null)
                        {
                        ...
                        if (restrictedStatuses.Contains(statusName) && eq.EmployeeId != null)
                        {
                ...
                var duplicateGroups = localData
                    .GroupBy(e => e.InventoryNumber)
                    .Where(g => g.Count() > 1);
            ...
            catch (System.Exception ex)
            {
                if (ex.InnerException?.Message.Contains("IX_Equipments_InventoryNumber") == true ||
                    ex.InnerException?.Message.Contains("UNIQUE constraint") == true)
                {
                    MessageBox.Show("Ошибка сохранения: в базе уже есть оборудование с таким инвентарным номером.", "Критическая ошибка", MessageBoxButton.OK, MessageBoxImage.Error);
                }
                else
                {
            ...
            try
            {
                ...
                if (saveFileDialog.ShowDialog() == true)
                {
                    using (var workbook = new XLWorkbook())
                    {
                        ...
                        worksheet.Cell(1, 7).Value = "Локация";
                        worksheet.Cell(1, 8).Value = "Цена";
                        worksheet.Cell(1, 9).Value = "Дата покупки";
                        worksheet.Cell(1, 10).Value = "Примечание";
                        worksheet.Cell(1, 11).Value = "Поставщик";
                        worksheet.Cell(1, 12).Value = "Срок службы (лет)";
                        worksheet.Cell(1, 13).Value = "Гарантия до";
                        ...
                        foreach (Equipment eq in _equipmentsView)
                        {
                            ...
                            if (eq.PurchaseDate.HasValue)
                                worksheet.Cell(row, 9).Value = eq.PurchaseDate.Value.ToString("dd.MM.yyyy");
                            worksheet.Cell(row, 10).Value = eq.Notes;
                            worksheet.Cell(row, 11).Value = eq.Supplier;
                            worksheet.Cell(row, 12).Value = eq.ServiceLifeYears;
                            if (eq.WarrantyExpireDate.HasValue)
                                worksheet.Cell(row, 13).Value = eq.WarrantyExpireDate.Value.ToString("dd.MM.yyyy");
